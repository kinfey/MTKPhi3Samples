...
RotaryEmbeddingMasterLut::RotaryEmbeddingMasterLut(const LLMType rotEmbType, const size_t length,
    ...
    : kType(rotEmbType), kTypeSize(getLLMTypeSize(kType)), kLength(length), kHeadDim(headDim),
      kRotBase(rotBase), kNtkScale(ntkScale) {
...
void RotaryEmbeddingMasterLut::setEmbed(std::vector<void*> rotEmbedBuffers, const size_t tokenIndex,
                                        const size_t tokenBatchSize, const size_t leftPadLength,
    ...
    switch (numRotEmbInputs) {
        case 1: {
            ...
            setEmbed(rotEmbInput, tokenIndex, tokenBatchSize, leftPadLength, rightPadLength);
            break;
        }
        case 2: {
            const auto rotEmbCosInput = rotEmbedBuffers[0];
            const auto rotEmbSinInput = rotEmbedBuffers[1];
            setEmbed(rotEmbCosInput, rotEmbSinInput, tokenIndex, tokenBatchSize,
...
void RotaryEmbeddingMasterLut::setEmbed(void* rotEmbedBuffer, const size_t tokenIndex,
    ...
    const auto masterLutStart = reinterpret_cast<char*>(mMasterLut) + tokenIndex * rowSizeBytes;

    DCHECK_GE(tokenBatchSize, leftPadLength + rightPadLength);
    const size_t numValidInputToken = tokenBatchSize - leftPadLength - rightPadLength;
    ...
    for (size_t i = 0; i < numValidInputToken; i++) {
        std::memcpy(curRotEmbedBuffer, masterLutStart + i * rowSizeBytes + cosOffset, copySize);
    ...
    // sin
    for (size_t i = 0; i < numValidInputToken; i++) {
        std::memcpy(curRotEmbedBuffer, masterLutStart + i * rowSizeBytes + sinOffset, copySize);
        curRotEmbedBuffer += copySize;
    }
...
void RotaryEmbeddingMasterLut::setEmbed(void* rotEmbedCosBuffer, void* rotEmbedSinBuffer,
                                        const size_t tokenIndex, const size_t tokenBatchSize,
    ...
    for (size_t i = 0; i < numValidInputToken; i++) {
        std::memcpy(curRotEmbedCosBuffer, masterLutStart + i * rowSizeBytes + cosOffset, copySize);
        std::memcpy(curRotEmbedSinBuffer, masterLutStart + i * rowSizeBytes + sinOffset, copySize);
...
void RotaryEmbeddingMasterLut::setEmbed(void* rotEmbedBuffer, const size_t tokenIndex,
    ...
    const auto requestedMaxIndex = tokenIndex + *std::max_element(positions.begin(),
                                                                  positions.end());
    const auto availableLength = getRotEmbedLength();
    if (requestedMaxIndex >= availableLength) {
        LOG(FATAL) << "Requested rotary embeddings (" << requestedMaxIndex << ") exceeds "
    ...
    for (const auto& pos : positions) {
        std::memcpy(curRotEmbedBuffer, masterLutStart + pos * rowSizeBytes + cosOffset, copySize);
        curRotEmbedBuffer += copySize;
    }

    // sin
    for (const auto& pos : positions) {
        std::memcpy(curRotEmbedBuffer, masterLutStart + pos * rowSizeBytes + sinOffset, copySize);
        curRotEmbedBuffer += copySize;
    }
}
...
void RotaryEmbeddingMasterLut::setEmbed(void* rotEmbedCosBuffer, void* rotEmbedSinBuffer,
    ...
    for (const auto& pos : positions) {
        std::memcpy(curRotEmbedCosBuffer, masterLutStart + pos * rowSizeBytes + cosOffset, copySize);
        std::memcpy(curRotEmbedSinBuffer, masterLutStart + pos * rowSizeBytes + sinOffset, copySize);
        curRotEmbedCosBuffer += copySize;
        curRotEmbedSinBuffer += copySize;
    }
}

size_t RotaryEmbeddingMasterLut::getRotEmbedSizeBytes(const size_t tokenBatchSize) const {
